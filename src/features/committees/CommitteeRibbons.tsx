...
export default function CommitteeRibbons({ onSelectCommittee }: CommitteeRibbonsProps) {
  return (
    <section id="committees" className="relative w-full bg-[#e5e5df] dark:bg-[#121215] border-t-2 border-[#d5d5cf] dark:border-[#27272a] pt-20 pb-20 px-4 mt-20 transition-colors">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 items-stretch">
        {COMMITTEES.map((comm) => {
          ...
          <div
            ...
            className={`ribbon-banner cursor-pointer relative bg-[#fafaf8] dark:bg-[#18181b] border-2 rounded-t-2xl p-6 pb-16 shadow-xl flex flex-col items-center text-center transition-all group ${comm.borderGlow}`}
          >
            {/* Top Color Accent Line matching logo */}
            <div className={`absolute top-0 inset-x-0 h-2.5 bg-gradient-to-r ${comm.accentColor} rounded-t-2xl`} />
            ...
            {/* Sub-badge */}
            <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full border mt-2 ${comm.badgeBg}`}>
              <Icon className={`w-5 h-5 ${comm.iconClassName}`} /> {comm.name}
            </span>
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-1">
              {comm.tags.map((tag, idx) => (
                <span 
                  key={idx} 
                  className="text-[10px] font-semibold px-2 py-0.5 rounded bg-[#e8e8e3] dark:bg-[#27272a] text-neutral-800 dark:text-neutral-300 border border-[#d5d5cf] dark:border-[#3f3f46]"
                >
                  {tag}
                </span>
              ))}
